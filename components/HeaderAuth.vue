    ...
    <div class="container flex flex-wrap   justify-between items-center mx-auto">  
      ...
      <div id="navbar-default" class=" w-full md:block md:w-auto " :class="phone ? '' : 'hidden' ">
        <ul v-if="user" class="flex flex-col p-4 mt-4 bg-gray-50 rounded-lg border border-gray-100 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-800 dark:border-gray-700">
          <li v-for="link in privateList" :key="link.id" :class="link.path === route.path ? 'block py-2 pr-4 pl-3 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white' : 'block py-2 pr-4 pl-3 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent'">
            <NuxtLink :to="link.path">
              {{ link.title }}
            </NuxtLink>
          </li> 
          <li class="block hover:cursor-pointer py-2 pr-4 pl-3 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent" @click="()=>{db.logOut()}">
            Log out  
          </li> 
...
import {publicList,privateList} from "../utils/routes"

    ...
    <nav class="border-gray-200 px-2 sm:px-4 py-2.5  dark:bg-gray-800">
      <div class="container flex flex-wrap   justify-between items-center mx-auto">  
        ...
        <div id="navbar-default" class="w-full md:block md:w-auto " :class="phone ? '' : 'hidden' ">
          <span v-for="link in privateList" :key="link.title" :class="link.path === route.path ? ' mx-2 py-2 pr-4 pl-3 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white' : 'mx-2 py-2 pr-4 pl-3 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent'">
            <NuxtLink :to="link.path">
              {{ link.title }}
            </NuxtLink>
          </span> 
          <span class="mx-2 hover:cursor-pointer py-2 pr-4 pl-3 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent" @click="()=>{db.logOut()}">
            Log out  
          </span> 
          <span class="mx-2 py-2 pr-4 pl-3 hover:select-none text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-gray-400 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent">
            {{ userinfo.email }}
          </span> 
        </div>
      </div> 
...
import {privateList} from "../utils/routes"
